...
<body>
  ...
  <div class="title mb-4">
    <h3>Chào <?php echo $user['firstName'] . " " . $user['lastName']; ?>, 
    Đây là giỏ hàng của bạn!</h3>
  </div>
  ...
  <div class="cart-details ">
            ...
            <li class="d-flex justify-content-between align-items-center mb-3 list-group-item">
              ...
              <div class="mt-1">
                <div class="d-flex flex-row justify-content-between align-items-start quantity-price">
                  <div>
                     <span class="item-price"><?= number_format($item['price'], 0, ',', '.') ?></span>đ x <span class="item-quantity"><?= $item['quantity'] ?></span>
                  </div>
                </div>
                <div class="d-flex flex-row justify-content-end align-items-end">
                  <span class="badge rounded-pill text-light p-2 mt-2 item-total-price" style="background-color: #fb4a36;"> <?= number_format($item['total_price'], 0, ',', '.') ?>đ</span>
                </div>
              </div>
    ...
    <div class="order-summary">
      ...
      <div class="summary-details payment">
        ...
        <div>
          <input type="radio" id="takeaway" name="payment_mode" value="Mang về" checked>
          <label for="Takeaway">Mua mang về</label>
        </div>
        <div>
          <input type="radio" id="cash" name="payment_mode" value="Tiền mặt">
          <label for="Cash">Tiền mặt</label>
        </div>
        <div>
          <input type="radio" id="card" name="payment_mode" value="Thẻ" disabled style="cursor: not-allowed;">
      ...
      <form id="checkout-form" action="order_review.php" method="post">
        ...
        <button type="button" id="checkout-button">Thanh toán</button>
      </form>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function updateSummary() {
        subtotalElement.textContent = number_format(subtotal);
        deliveryFeeElement.textContent = number_format(deliveryFee);
        totalElement.textContent = number_format(subtotal + deliveryFee);
      }
      ...
      function updateDeliveryFee() {
        ...
        deliveryFee = selectedPaymentMode === 'Mang về' ? 0 : 13000;
        updateSummary();
      }
      ...
      function updateQuantity(element, change, isInput = false) {
        ...
        itemContainer.querySelector('.item-total-price').textContent = itemTotalPrice + 'đ';
