...
$deliveryFee = ($_POST['payment_mode'] === 'Mang về') ? 0 : 13000;
...
<body>
  ...
  <div class="title mt-2">
    <h3>Chào <?php echo $user['firstName'] . " " . $user['lastName']; ?>, hoàn thành đơn hàng của bạn!</h3>
  </div>
  <div class=" main mt-4">
    <div class="order-fee">
    ...
    <div class="order-summary">
          ...
          <div class="order-item d-flex align-items-center">
            ...
            <div class="ms-1 row d-flex justify-content-between w-100">
              <div class="col d-flex flex-column justify-content-center ">
                ...
                <div class="d-flex flex-row ">Số lượng: <?= htmlspecialchars($item['quantity']) ?></div>
              </div>
              <div class="col d-flex flex-column justify-content-center">
                <div class="d-flex flex-row justify-content-end align-items-center mt-2">  <?= htmlspecialchars($item['price'], 0) ?>đ x <?= htmlspecialchars($item['quantity']) ?></div>
                <div class="d-flex flex-row justify-content-end align-items-start mb-2">
                  <span class="badge rounded-pill text-light p-2 mt-2 item-total-price" style="background-color: #fb4a36;"> <?= $item['total_price'] ?>đ</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <h4 class="mt-1 ">Phí đơn hàng</h4>
      <hr>
      <div class="summary-details">
        <div class="fee-details">
          <div><strong>Tổng phụ:</strong></div>
          <div> <?= str_replace(',', '.', number_format($subtotal)) ?>đ</div>
        </div>
        <div class="fee-details">
        ...
        <div class="fee-details">
          ...
          <div> <?= str_replace(',', '.', number_format($deliveryFee)) ?>đ</div>
        </div>
        <div class="fee-details">
          <div><strong>Thành tiền:</strong></div>
          <div> <?= str_replace(',', '.', number_format($total)) ?>đ</div>
        </div>
      </div>
      <hr>
      <?php
